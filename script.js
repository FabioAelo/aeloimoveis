...
const open=()=>{panel.classList.add('open');panel.setAttribute('aria-hidden','false');if(!started){started=true;add('Olá! Sou o Assistente AELO. Posso ajudar você a encontrar um imóvel, anunciar seu imóvel ou falar sobre avaliação imobiliária. Como posso ajudar?');main()}input.focus()}; const shut=()=>{panel.classList.remove('open');panel.setAttribute('aria-hidden','true')};
const main=()=>buttons([{label:'🏠 Comprar imóvel',action:buy},{label:'🔑 Alugar imóvel',action:rent},{label:'💰 Vender meu imóvel',action:sell},{label:'📊 Avaliação / PTAM',action:valuation},{label:'⚖️ Perícia / assistência',action:expert},{label:'📱 Falar com Fabio',action:contact}]);
