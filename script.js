...
const seasonClosingReview=(p,ctx)=>{
  ...
  const summary=`<div class="aelo-lead-card"><strong>${esc(p.title)}</strong><small>${esc(p.location||'')}<br>📅 ${esc(period)}<br>👥 ${ctx.guests||'A definir'} hóspedes · ${ctx.adults||0} adultos · ${ctx.children||0} crianças · ${ctx.babies||0} bebês<br>🐾 ${ctx.hasPet?`${ctx.petCount||1} pet(s) ${ctx.petSize||''}`:'Sem pet'} · ${ctx.hasSmoker?'🚬 fumante':'🚭 não fumante'}${ctx.estimatedTotal!==null&&ctx.estimatedTotal!==undefined?`<br>💰 ${fmtBRL(Number(ctx.estimatedTotal))} estimados`:''}<br>👤 ${esc(ctx.name)}<br>📱 ${esc(ctx.phone)}</small>${ctx.note?`<small>📝 ${esc(ctx.note)}</small>`:''}</div>`;
...
const seasonWaMessage=(p,ctx)=>`Olá, Fábio! Vim pelo site AELO e gostaria de solicitar a hospedagem do imóvel ${p.title}${p.location?` em ${p.location}`:''}. Período: ${ctx.checkin||'a definir'} a ${ctx.checkout||'a definir'}. Hóspedes: ${ctx.guests||'a definir'} (${ctx.adults||0} adultos, ${ctx.children||0} crianças, ${ctx.babies||0} bebês). Pet: ${ctx.hasPet?`${ctx.petCount||1} ${ctx.petSize||''}`:'não'}. Fumante: ${ctx.hasSmoker?'sim':'não'}. ${ctx.estimatedTotal!==null&&ctx.estimatedTotal!==undefined?`Valor estimado informado no site: ${fmtBRL(Number(ctx.estimatedTotal))}. `:''}Meu nome é ${ctx.name||'a informar'} e meu WhatsApp é ${ctx.phone||'a informar'}.${ctx.note?` Observação: ${ctx.note}`:''}`;
