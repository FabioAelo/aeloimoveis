...
const seasonGuestsStep=(ctx)=>{
  add('Quem vai se hospedar? Informe adultos, crianças e bebês para eu buscar uma hospedagem adequada.');
  const wrap=document.createElement('div'); wrap.className='aelo-season-guest-picker';
  wrap.innerHTML=`<div class="aelo-guest-row"><span>👤 Adultos</span><div><button type="button" data-dec="adults">−</button><b data-count="adults">2</b><button type="button" data-inc="adults">+</button></div></div><div class="aelo-guest-row"><span>🧒 Crianças</span><div><button type="button" data-dec="children">−</button><b data-count="children">0</b><button type="button" data-inc="children">+</button></div></div><div class="aelo-guest-row"><span>👶 Bebês <small>(até 2 anos)</small></span><div><button type="button" data-dec="babies">−</button><b data-count="babies">0</b><button type="button" data-inc="babies">+</button></div></div><button type="button" class="aelo-guest-continue">Continuar</button><p class="aelo-lead-status"></p>`;
  quick.innerHTML=''; quick.appendChild(wrap);
  const counts={adults:2,children:0,babies:0};
  const refresh=()=>Object.keys(counts).forEach(k=>wrap.querySelector(`[data-count="${k}"]`).textContent=counts[k]);
  wrap.querySelectorAll('[data-inc]').forEach(b=>b.onclick=()=>{const k=b.dataset.inc;counts[k]=Math.min(k==='adults'?20:10,counts[k]+1);refresh()});
  wrap.querySelectorAll('[data-dec]').forEach(b=>b.onclick=()=>{const k=b.dataset.dec;const min=k==='adults'?1:0;counts[k]=Math.max(min,counts[k]-1);refresh()});
  wrap.querySelector('.aelo-guest-continue').onclick=()=>{const guests=counts.adults+counts.children+counts.babies;if(!guests){wrap.querySelector('.aelo-lead-status').textContent='Informe pelo menos 1 hóspede.';return;}seasonPetStep({...ctx,...counts,guests});};
};
const seasonPetStep=(ctx)=>{add('Vai levar algum animal de estimação?');buttons([{label:'🐾 Não',action:()=>seasonSmokingStep({...ctx,hasPet:false,petCount:0,petSize:''})},{label:'🐾 Sim',action:()=>seasonPetDetailsStep(ctx)}]);};
const seasonPetDetailsStep=(ctx)=>{add('Quantos animais e qual o porte?');const wrap=document.createElement('div');wrap.className='aelo-season-guest-picker';wrap.innerHTML=`<div class="aelo-guest-row"><span>🐾 Quantidade</span><div><button type="button" data-dec="pet">−</button><b data-count="pet">1</b><button type="button" data-inc="pet">+</button></div></div><div class="aelo-pet-size"><span>Porte</span><div><button type="button" data-size="Pequeno">Pequeno</button><button type="button" data-size="Médio">Médio</button><button type="button" data-size="Grande">Grande</button></div></div><button type="button" class="aelo-guest-continue">Continuar</button><p class="aelo-lead-status"></p>`;quick.innerHTML='';quick.appendChild(wrap);let count=1,size='Pequeno';const refresh=()=>wrap.querySelector('[data-count="pet"]').textContent=count;wrap.querySelector('[data-inc]').onclick=()=>{count=Math.min(5,count+1);refresh()};wrap.querySelector('[data-dec]').onclick=()=>{count=Math.max(1,count-1);refresh()};wrap.querySelectorAll('[data-size]').forEach(b=>b.onclick=()=>{size=b.dataset.size;wrap.querySelectorAll('[data-size]').forEach(x=>x.classList.toggle('selected',x===b))});wrap.querySelector('[data-size="Pequeno"]').classList.add('selected');wrap.querySelector('.aelo-guest-continue').onclick=()=>seasonSmokingStep({...ctx,hasPet:true,petCount:count,petSize:size});};
const seasonSmokingStep=(ctx)=>{add('Algum hóspede é fumante?');buttons([{label:'🚭 Não',action:()=>seasonBudgetStep({...ctx,hasSmoker:false})},{label:'🚬 Sim',action:()=>seasonBudgetStep({...ctx,hasSmoker:true})}]);};
...
const text=t=>{
  ...
  delete p.intent; if(p.propertyType==='Terreno'||chatCtx.propertyType==='Terreno'){delete p.bedrooms;} mergeCtx(p);
...
const petRank=v=>({pequeno:1,médio:2,medio:2,grande:3}[String(v||'').toLowerCase()]||0);
const seasonRuleCheck=(p,ctx)=>{const issues=[];const max=Number(p.max_guests||0);if(max>0&&Number(ctx.guests||0)>max)issues.push(`Este imóvel comporta no máximo ${max} hóspedes.`);if(ctx.children>0&&p.allow_children===false)issues.push('Este imóvel não aceita crianças.');if(ctx.babies>0&&p.allow_babies===false)issues.push('Este imóvel não aceita bebês.');if(ctx.hasPet&&p.allow_pets===false)issues.push('Este imóvel não aceita animais de estimação.');if(ctx.hasPet&&p.allow_pets!==false&&p.max_pet_size&&petRank(ctx.petSize)>petRank(p.max_pet_size))issues.push(`Este imóvel aceita pets até porte ${p.max_pet_size}.`);if(ctx.hasSmoker&&p.allow_smoking===false)issues.push('Este imóvel não permite fumar.');if(ctx.hasParty&&p.allow_parties===false)issues.push('Este imóvel não permite festas ou eventos.');return issues;};
const showSeasonRuleIssues=(p,ctx,issues)=>{add(`<div class="aelo-rule-alert"><strong>Antes de continuar</strong><br>${issues.map(esc).join('<br>')}</div>`,'bot',true);buttons([{label:'↩️ Alterar informações',action:()=>seasonClosingStart(p,ctx)},{label:'💬 Falar com o corretor',action:()=>wa(seasonWaMessage(p,ctx))}]);};
const seasonClosingStart=(p,ctx)=>{ seasonClosingGuests(p,ctx); };
const seasonClosingGuests=(p,ctx)=>{
  add('Antes de concluir, quem vai se hospedar? Informe adultos, crianças e bebês.','bot');
  const wrap=document.createElement('div');wrap.className='aelo-season-guest-picker';
  const adults=Math.max(1,Number(ctx.adults||ctx.guests||2)), children=Math.max(0,Number(ctx.children||0)), babies=Math.max(0,Number(ctx.babies||0));
  wrap.innerHTML=`<div class="aelo-guest-row"><span>👤 Adultos</span><div><button type="button" data-dec="adults">−</button><b data-count="adults">${adults}</b><button type="button" data-inc="adults">+</button></div></div><div class="aelo-guest-row"><span>🧒 Crianças</span><div><button type="button" data-dec="children">−</button><b data-count="children">${children}</b><button type="button" data-inc="children">+</button></div></div><div class="aelo-guest-row"><span>👶 Bebês <small>(até 2 anos)</small></span><div><button type="button" data-dec="babies">−</button><b data-count="babies">${babies}</b><button type="button" data-inc="babies">+</button></div></div><button type="button" class="aelo-guest-continue">Continuar</button>`;
  quick.innerHTML='';quick.appendChild(wrap);const counts={adults,children,babies};const refresh=()=>Object.keys(counts).forEach(k=>wrap.querySelector(`[data-count="${k}"]`).textContent=counts[k]);wrap.querySelectorAll('[data-inc]').forEach(b=>b.onclick=()=>{const k=b.dataset.inc;counts[k]=Math.min(k==='adults'?20:10,counts[k]+1);refresh()});wrap.querySelectorAll('[data-dec]').forEach(b=>b.onclick=()=>{const k=b.dataset.dec;counts[k]=Math.max(k==='adults'?1:0,counts[k]-1);refresh()});wrap.querySelector('.aelo-guest-continue').onclick=()=>{const next={...ctx,...counts,guests:counts.adults+counts.children+counts.babies};const issues=seasonRuleCheck(p,next);if(issues.length){showSeasonRuleIssues(p,next,issues);return;}seasonClosingPet(p,next);};
};
const seasonClosingPet=(p,ctx)=>{add('Vai levar algum animal de estimação?','bot');buttons([{label:'🐾 Não',action:()=>seasonClosingSmoking(p,{...ctx,hasPet:false,petCount:0,petSize:''})},{label:'🐾 Sim',action:()=>seasonClosingPetDetails(p,ctx)}]);};
const seasonClosingPetDetails=(p,ctx)=>{add('Quantos animais e qual o porte?','bot');const wrap=document.createElement('div');wrap.className='aelo-season-guest-picker';wrap.innerHTML=`<div class="aelo-guest-row"><span>🐾 Quantidade</span><div><button type="button" data-dec="pet">−</button><b data-count="pet">1</b><button type="button" data-inc="pet">+</button></div></div><div class="aelo-pet-size"><span>Porte</span><div><button type="button" data-size="Pequeno">Pequeno</button><button type="button" data-size="Médio">Médio</button><button type="button" data-size="Grande">Grande</button></div></div><button type="button" class="aelo-guest-continue">Continuar</button>`;quick.innerHTML='';quick.appendChild(wrap);let count=1,size='Pequeno';wrap.querySelector('[data-size="Pequeno"]').classList.add('selected');wrap.querySelector('[data-inc]').onclick=()=>{count=Math.min(5,count+1);wrap.querySelector('[data-count="pet"]').textContent=count};wrap.querySelector('[data-dec]').onclick=()=>{count=Math.max(1,count-1);wrap.querySelector('[data-count="pet"]').textContent=count};wrap.querySelectorAll('[data-size]').forEach(b=>b.onclick=()=>{size=b.dataset.size;wrap.querySelectorAll('[data-size]').forEach(x=>x.classList.toggle('selected',x===b))});wrap.querySelector('.aelo-guest-continue').onclick=()=>{const next={...ctx,hasPet:true,petCount:count,petSize:size};const issues=seasonRuleCheck(p,next);if(issues.length){showSeasonRuleIssues(p,next,issues);return;}seasonClosingSmoking(p,next);};};
const seasonClosingSmoking=(p,ctx)=>{add('Algum hóspede é fumante?');buttons([{label:'🚭 Não',action:()=>seasonClosingParty(p,{...ctx,hasSmoker:false})},{label:'🚬 Sim',action:()=>{const next={...ctx,hasSmoker:true};const issues=seasonRuleCheck(p,next);if(issues.length){showSeasonRuleIssues(p,next,issues);return;}seasonClosingParty(p,next);}}]);};
const seasonClosingParty=(p,ctx)=>{add('Pretende realizar alguma festa ou evento durante a estadia?','bot');buttons([{label:'🎉 Não',action:()=>seasonClosingName(p,{...ctx,hasParty:false})},{label:'🎉 Sim',action:()=>{const next={...ctx,hasParty:true};const issues=seasonRuleCheck(p,next);if(issues.length){showSeasonRuleIssues(p,next,issues);return;}seasonClosingName(p,next);}}]);};
const seasonClosingName=(p,ctx)=>{add('Ótimo. Agora, como posso chamar você?','bot');const wrap=document.createElement('div');wrap.className='aelo-season-lead-step';wrap.innerHTML=`<label>Seu nome<input class="season-lead-name" type="text" autocomplete="name" placeholder="Digite seu nome"></label><button type="button" class="aelo-lead-submit season-next-name">Continuar</button><p class="aelo-lead-status" role="status"></p>`;quick.innerHTML='';quick.appendChild(wrap);wrap.querySelector('.season-next-name').onclick=()=>{const name=wrap.querySelector('.season-lead-name').value.trim();if(!name){wrap.querySelector('.aelo-lead-status').textContent='Informe seu nome para continuar.';return;}seasonClosingPhone(p,{...ctx,name});};};
...
const seasonClosingReview=(p,ctx)=>{
  ...
  const summary=`<div class="aelo-lead-card"><strong>${esc(p.title)}</strong><small>${esc(p.location||'')}<br>📅 ${esc(period)}<br>👥 ${ctx.guests||'A definir'} hóspedes · ${ctx.adults||0} adultos · ${ctx.children||0} crianças · ${ctx.babies||0} bebês<br>🐾 ${ctx.hasPet?`${ctx.petCount||1} pet(s) ${ctx.petSize||''}`:'Sem pet'} · ${ctx.hasSmoker?'🚬 fumante':'🚭 não fumante'} · ${ctx.hasParty?'🎉 evento':'🎉 sem evento'}${ctx.estimatedTotal!==null&&ctx.estimatedTotal!==undefined?`<br>💰 ${fmtBRL(Number(ctx.estimatedTotal))} estimados`:''}<br>👤 ${esc(ctx.name)}<br>📱 ${esc(ctx.phone)}</small>${ctx.note?`<small>📝 ${esc(ctx.note)}</small>`:''}</div>`;
...
const seasonWaMessage=(p,ctx)=>`Olá, Fábio! Vim pelo site AELO e gostaria de solicitar a hospedagem do imóvel ${p.title}${p.location?` em ${p.location}`:''}. Período: ${ctx.checkin||'a definir'} a ${ctx.checkout||'a definir'}. Hóspedes: ${ctx.guests||'a definir'} (${ctx.adults||0} adultos, ${ctx.children||0} crianças, ${ctx.babies||0} bebês). Pet: ${ctx.hasPet?`${ctx.petCount||1} ${ctx.petSize||''}`:'não'}. Fumante: ${ctx.hasSmoker?'sim':'não'}. Evento/festa: ${ctx.hasParty?'sim':'não'}. ${ctx.estimatedTotal!==null&&ctx.estimatedTotal!==undefined?`Valor estimado informado no site: ${fmtBRL(Number(ctx.estimatedTotal))}. `:''}Meu nome é ${ctx.name||'a informar'} e meu WhatsApp é ${ctx.phone||'a informar'}.${ctx.note?` Observação: ${ctx.note}`:''}`;
...
const seasonSubmitLead=async(p,ctx)=>{
  ...
  const detail=[`Imóvel: ${p.title}`,`Localização: ${p.location||'não informada'}`,`Check-in: ${ctx.checkin||'a definir'}`,`Check-out: ${ctx.checkout||'a definir'}`,`Hóspedes: ${ctx.guests||'a definir'} (${ctx.adults||0} adultos, ${ctx.children||0} crianças, ${ctx.babies||0} bebês)`,`Pet: ${ctx.hasPet?'sim':'não'}${ctx.hasPet?` · ${ctx.petCount||1} · porte ${ctx.petSize||'não informado'}`:''}`,`Fumante: ${ctx.hasSmoker?'sim':'não'}`,`Festa/evento: ${ctx.hasParty?'sim':'não'}`,finalEstimate?`Valor estimado: ${fmtBRL(finalEstimate)}`:null,ctx.note?`Observação: ${ctx.note}`:null].filter(Boolean).join(' | ');
  ...
  const reservationPayload={property_id:p.id,lead_id:lead?.id||null,guest_name:ctx.name,guest_whatsapp:ctx.phone,checkin:ctx.checkin||null,checkout:ctx.checkout||null,guests:Number(ctx.guests||2),adults:Number(ctx.adults||0),children:Number(ctx.children||0),babies:Number(ctx.babies||0),has_pet:!!ctx.hasPet,pet_count:Number(ctx.petCount||0),pet_size:ctx.petSize||null,has_smoker:!!ctx.hasSmoker,estimated_total:finalEstimate||null,note:ctx.note||null,status:'solicitada'};
