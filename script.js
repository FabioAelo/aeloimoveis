...
/* V25 — Assistente AELO: captação de leads */
...
const buy=()=>{add('Ótimo. Posso procurar imóveis disponíveis no catálogo AELO. Qual faixa de valor você procura?');buttons([{label:'Até R$ 500 mil',action:()=>search('venda',p=>p<=500000,'até R$ 500 mil','Compra')},{label:'R$ 500 mil a R$ 1 milhão',action:()=>search('venda',p=>p>=500000&&p<=1000000,'R$ 500 mil a R$ 1 milhão','Compra')},{label:'Acima de R$ 1 milhão',action:()=>search('venda',p=>p>=1000000,'acima de R$ 1 milhão','Compra')},{label:'Escolher região',action:()=>region('venda','Compra')}])};
const rent=()=>{add('Perfeito. Vamos procurar imóveis para aluguel. Qual valor mensal máximo?');buttons([{label:'Até R$ 3 mil',action:()=>search('aluguel',p=>p<=3000,'até R$ 3 mil/mês','Aluguel')},{label:'R$ 3 mil a R$ 6 mil',action:()=>search('aluguel',p=>p>=3000&&p<=6000,'R$ 3 mil a R$ 6 mil/mês','Aluguel')},{label:'Acima de R$ 6 mil',action:()=>search('aluguel',p=>p>=6000,'acima de R$ 6 mil/mês','Aluguel')},{label:'Escolher região',action:()=>region('aluguel','Aluguel')}])};
...
const search=(type,pred,label,interest)=>show(properties.filter(p=>p.type===type&&pred(Number(p.price||0))),label,interest); const searchRegion=(type,term,interest)=>show(properties.filter(p=>p.type===type&&String(p.location||'').toLowerCase().includes(term)),term,interest);
const show=(list,label,interest)=>{if(!list.length){add(`Não encontrei imóveis publicados no catálogo para “${label}”. Posso encaminhar você ao Fabio para receber outras opções.`);buttons([{label:'📋 Receber opções',action:()=>collectLead(interest||'Compra')},{label:'📱 Falar com Fabio',action:()=>wa('Olá, Fabio! Vim pelo Assistente AELO e gostaria de receber opções de imóveis.')},{label:'↩️ Voltar',action:main}]);return} add(`Encontrei ${list.length} opção(ões) para “${label}”. Veja abaixo:`);list.slice(0,4).forEach(p=>{add(`<div class="aelo-chat-property"><img src="${esc(p.image_url||'logo.png')}" alt="${esc(p.title)}"><strong>${esc(p.title)}</strong><small>${esc(p.location||'')}<br>${esc(p.meta.join(' • '))}</small><b>${esc(p.price_label||'')}</b><br><a href="#imoveis" data-prop-id="${esc(p.id)}">Ver imóvel →</a></div>`,'bot',true)});document.querySelectorAll('[data-prop-id]').forEach(a=>a.onclick=e=>{e.preventDefault();shut();openModal(a.dataset.propId)});buttons([{label:'📋 Quero receber opções',action:()=>collectLead(interest||'Compra')},{label:'📱 Tenho interesse',action:()=>wa('Olá, Fabio! Vi um imóvel no site AELO e gostaria de receber mais informações.')},{label:'↩️ Nova busca',action:main}])};
const collectLead=(interest)=>{quick.innerHTML=''; const wrap=document.createElement('div'); wrap.className='aelo-lead-card'; wrap.innerHTML=`<strong>Vamos deixar seu contato?</strong><small>Assim o Fabio pode retornar com atendimento personalizado.</small><label>Nome<input class="lead-name" type="text" autocomplete="name" placeholder="Seu nome"></label><label>WhatsApp<input class="lead-phone" type="tel" autocomplete="tel" placeholder="(71) 99999-9999"></label><label>Região de interesse<input class="lead-region" type="text" placeholder="Ex.: Lauro de Freitas"></label><label>Mensagem <span>(opcional)</span><textarea class="lead-message" rows="2" placeholder="Conte brevemente o que procura"></textarea></label><button type="button" class="aelo-lead-submit">Enviar meu contato</button><p class="aelo-lead-status" role="status"></p>`; quick.appendChild(wrap); requestAnimationFrame(()=>{quick.scrollTop=quick.scrollHeight;}); const btn=wrap.querySelector('.aelo-lead-submit'); btn.onclick=async()=>{const name=wrap.querySelector('.lead-name').value.trim(),phone=wrap.querySelector('.lead-phone').value.trim(),region=wrap.querySelector('.lead-region').value.trim(),message=wrap.querySelector('.lead-message').value.trim(),status=wrap.querySelector('.aelo-lead-status'); if(!name||!phone){status.textContent='Informe seu nome e WhatsApp.';return} btn.disabled=true;btn.textContent='Enviando...'; const client=getSupabaseClient(); if(!client){status.textContent='Não foi possível conectar ao atendimento. Use o WhatsApp.';btn.disabled=false;btn.textContent='Enviar meu contato';return} const {error}=await client.from('leads').insert({name,whatsapp:phone,interest,region:region||null,message:message||null,source:'site-chatbot'}); if(error){console.error(error);status.textContent='Não foi possível registrar agora. Você pode falar pelo WhatsApp.';btn.disabled=false;btn.textContent='Tentar novamente';return} add(`Perfeito, ${name.split(' ')[0]}! Seu contato foi recebido. O Fabio poderá retornar pelo WhatsApp. 📲`); buttons([{label:'📱 Falar com Fabio',action:()=>wa(`Olá, Fabio! Acabei de deixar meu contato pelo site AELO. Meu nome é ${name}. Interesse: ${interest}.` )},{label:'↩️ Menu principal',action:main}]); }};
