...
const budgetStep=(interest,type,budgetLabel,budgetMax,budgetMin=0)=>{add('Em qual região você procura?');buttons([{label:'Lauro de Freitas',action:()=>bedroomStep({interest,type,budgetLabel,budgetMin,budgetMax,region:'Lauro de Freitas'})},{label:'Camaçari',action:()=>bedroomStep({interest,type,budgetLabel,budgetMin,budgetMax,region:'Camaçari'})},{label:'Salvador',action:()=>bedroomStep({interest,type,budgetLabel,budgetMin,budgetMax,region:'Salvador'})},{label:'Outra região',action:()=>freeRegionStep({interest,type,budgetLabel,budgetMin,budgetMax})}])};
const freeRegionStep=(ctx)=>{add('Digite a região que você procura no campo abaixo.');input.placeholder='Ex.: Vilas do Atlântico';buttons([{label:'Continuar',action:()=>{const region=(input.value||'').trim();input.value='';input.placeholder='Digite sua mensagem...';bedroomStep({...ctx,region:region||'Não informada'})}}])};
const bedroomStep=(ctx)=>{add('Quantos quartos você precisa?');buttons([{label:'1 quarto',action:()=>finishSearch({...ctx,bedrooms:1})},{label:'2 quartos',action:()=>finishSearch({...ctx,bedrooms:2})},{label:'3 quartos',action:()=>finishSearch({...ctx,bedrooms:3})},{label:'4+ quartos',action:()=>finishSearch({...ctx,bedrooms:4})},{label:'Não é importante',action:()=>finishSearch({...ctx,bedrooms:0})}])};
