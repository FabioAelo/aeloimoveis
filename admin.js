...
window.openWhatsAppLead = async id => {
  ...
  const {error}=await client.from('lead_interactions').insert({lead_id:id,type:'whatsapp',note:`WhatsApp iniciado — ${finalMessage}`});
  if(error) return alert('Não foi possível registrar o contato no histórico: '+error.message);
  window.open(`https://wa.me/55${wa}?text=${encodeURIComponent(finalMessage)}`,'_blank','noopener');
  await refreshLeads();
};
