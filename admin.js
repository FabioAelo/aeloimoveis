...
function setUserForm(profile,perm){bindUserRoleDefaults();$('userProfileId').value=profile?.user_id||'';$('userProfileName').value=profile?.full_name||'';$('userProfileEmail').value=profile?.email||'';$('userProfileRole').value=profile?.role||'consulta';$('userProfileActive').checked=profile?.is_active!==false;const d=perm||ROLE_DEFAULTS[profile?.role]||ROLE_DEFAULTS.consulta;Object.entries(d).forEach(([k,v])=>{const id='perm'+k.split('_').map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join('');const el=$(id);if(el)el.checked=!!v;});}
...
async function saveUserProfile(){const id=$('userProfileId').value;if(!id)return showMsg('userProfileMsg','Selecione um usuário da lista antes de salvar.');const profile={user_id:id,full_name:$('userProfileName').value.trim(),email:$('userProfileEmail').value.trim(),role:$('userProfileRole').value,is_active:$('userProfileActive').checked,updated_at:new Date().toISOString()};const a=await client.from('user_profiles').upsert(profile,{onConflict:'user_id'});if(a.error)return showMsg('userProfileMsg',a.error.message);const b=await client.from('user_permissions').upsert({...getUserPermissionPayload(),user_id:id,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(b.error)return showMsg('userProfileMsg',b.error.message);const me=await client.auth.getUser();if(me?.data?.user?.id){await client.from('audit_log').insert({user_id:me.data.user.id,action:'atualizou_permissoes',module:'usuarios',record_id:id,details:{role:profile.role}});}showMsg('userProfileMsg','Permissões salvas com sucesso.');await refreshUsers();}
function clearUserProfile(){setUserForm({role:'consulta',is_active:true},{...ROLE_DEFAULTS.consulta});$('userProfileId').value='';$('userProfileEmail').value='';showMsg('userProfileMsg','');}
async function syncUsers(){const msg=$('userProfileMsg');if(msg) msg.textContent='Sincronizando contas do Supabase Auth...';const r=await client.rpc('aelo_sync_auth_profiles');if(r.error){if(msg) msg.textContent=r.error.message;return;}if(msg) msg.textContent='Contas sincronizadas. Agora elas aparecem na lista.';await refreshUsers();}
function bindUserRoleDefaults(){
  const role=$('userProfileRole');
  if(!role || role.dataset.defaultsBound==='1') return;
  role.dataset.defaultsBound='1';
  role.addEventListener('change',()=>{
    applyRoleDefaults(role.value);
    showMsg('userProfileMsg','Permissões padrão do perfil '+(role.options[role.selectedIndex]?.text||role.value)+' aplicadas. Clique em Salvar permissões.');
  });
}

function initUsersManager(){ bindUserRoleDefaults(); $('refreshUsers')?.addEventListener('click',refreshUsers);$('syncUsers')?.addEventListener('click',syncUsers);$('saveUserProfile')?.addEventListener('click',saveUserProfile);$('clearUserProfile')?.addEventListener('click',clearUserProfile);$('userProfileRole')?.addEventListener('change',e=>applyRoleDefaults(e.target.value));refreshUsers();}
