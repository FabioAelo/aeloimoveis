...
async function loadLeadAssignees(){
  LEAD_ASSIGNEES=[];
  if(!client) return;
  const {data,error}=await client.from('user_profiles').select('user_id,full_name,email,role,is_active').eq('is_active',true).order('full_name',{ascending:true});
  if(!error) LEAD_ASSIGNEES=data||[];
  populateLeadAssigneeFilter();
}
function leadAssigneeName(id){
  if(!id) return 'Sem responsável';
  const u=LEAD_ASSIGNEES.find(x=>x.user_id===id);
  return u?.full_name || u?.email || 'Responsável';
}
function populateLeadAssigneeFilter(){
  const sel=document.getElementById('leadAssigneeFilter'); if(!sel) return;
  const current=sel.value||'todos';
  const opts=LEAD_ASSIGNEES.map(u=>`<option value="${escapeHtml(u.user_id)}">${escapeHtml(u.full_name||u.email||'Usuário')}</option>`).join('');
  sel.innerHTML='<option value="todos">Todos</option>'+opts;
...
window.focusLead=id=>{
  ...
  ['leadSearch','leadStatusFilter','leadInterestFilter','leadRegionFilter','leadAssigneeFilter','leadBedroomsFilter'].forEach(k=>{
...
function applyLeadFilters(){
  ...
  const af=document.getElementById('leadAssigneeFilter')?.value||'todos';
  ...
  const filtered=allLeads.filter(l=>{
    ...
    return quick && (!search||hay.includes(search)) && (sf==='todos'||(l.status||'novo')===sf) && (inf==='todos'||String(l.interest||'').toLowerCase()===inf.toLowerCase()) && (rf==='todos'||String(l.region||'')===rf) && (af==='todos'||String(l.assigned_user_id||'')===af) && (bf==='todos'||bedrooms>=Number(bf));
  });
...
window.markLeadContacted = async id => {
  if (!can('edit_leads')) return alert('Seu usuário não possui permissão para gerenciar leads.');
...
window.saveLead = async id => {
  ...
  const updatePayload={status, notes, next_follow_up_at, last_contact_at};
  if(can('edit_leads')) updatePayload.assigned_user_id = $("assignee-"+id)?.value || null;
  const { error } = await client.from("leads").update(updatePayload).eq("id", id);
...
function setUserForm(profile,perm){bindUserRoleDefaults();$('userProfileId').value=profile?.user_id||'';$('userProfileName').value=profile?.full_name||'';$('userProfileEmail').value=profile?.email||'';$('userProfileRole').value=profile?.role||'consulta';$('userProfilePassword').value='';$('userProfileActive').checked=profile?.is_active!==false;const d=perm||ROLE_DEFAULTS[profile?.role]||ROLE_DEFAULTS.consulta;Object.entries(d).forEach(([k,v])=>{const id='perm'+k.split('_').map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join('');const el=$(id);if(el)el.checked=!!v;});}
async function refreshUsers(){const msg=$('userProfileList');if(!msg||!client)return;msg.innerHTML='<div class="reservation-empty">Carregando usuários...</div>';const {data,error}=await client.from('user_profiles').select('*').order('full_name',{ascending:true});if(error){msg.innerHTML='<div class="reservation-empty"><strong>Não foi possível carregar os usuários.</strong><br>'+escapeHtml(error.message)+'</div>';return;}const ids=(data||[]).map(x=>x.user_id);let perms=[];if(ids.length){const r=await client.from('user_permissions').select('*').in('user_id',ids);perms=r.data||[];}const pm=new Map(perms.map(x=>[x.user_id,x]));msg.innerHTML=(data||[]).length?(data||[]).map(u=>{const roleLabels={admin:'Administrador',gerente:'Gerente',corretor:'Corretor',atendimento:'Atendimento',financeiro:'Financeiro',consulta:'Consulta'};return `<article class="user-profile-row"><div><strong>${escapeHtml(u.full_name||'Sem nome')}</strong><small>${escapeHtml(u.email||'')}</small></div><span class="user-role-pill">${roleLabels[u.role]||u.role}</span><span class="user-active-pill ${u.is_active?'on':'off'}">${u.is_active?'● Ativo':'● Desativado'}</span><small>${Object.values(pm.get(u.user_id)||{}).filter(v=>v===true).length} permissões</small><button type="button" class="ghost mini" onclick="editUserProfile('${u.user_id}')">Editar</button></article>`}).join(''):'<div class="reservation-empty">Nenhum perfil cadastrado.</div>';}
window.editUserProfile=async id=>{const a=await client.from('user_profiles').select('*').eq('user_id',id).single();const b=await client.from('user_permissions').select('*').eq('user_id',id).maybeSingle();if(a.error)return showMsg('userProfileMsg',a.error.message);setUserForm(a.data,b.data||null);$('deleteUserProfile').disabled=false;$('userProfileName')?.focus();};
async function callUserAdmin(action, extra={}){const {data:{session}}=await client.auth.getSession();if(!session?.access_token)return {error:'Sessão expirada. Faça login novamente.'};const url=(window.AELO_SUPABASE_CONFIG?.url||'').replace(/\/$/,'')+'/functions/v1/admin-manage-user';try{const r=await fetch(url,{method:'POST',headers:{Authorization:'Bearer '+session.access_token,'apikey':window.AELO_SUPABASE_CONFIG.anonKey,'Content-Type':'application/json'},body:JSON.stringify({action,...extra})});const j=await r.json().catch(()=>({}));return r.ok?j:{error:j.error||('Erro HTTP '+r.status)};}catch(e){return {error:e.message||'Não foi possível acessar a central segura de usuários.'};}}
async function saveUserProfile(){const id=$('userProfileId').value;const email=$('userProfileEmail').value.trim().toLowerCase();const name=$('userProfileName').value.trim();const role=$('userProfileRole').value;const active=$('userProfileActive').checked;const password=$('userProfilePassword').value;const permissions=getUserPermissionPayload();if(!email)return showMsg('userProfileMsg','Informe o e-mail.');if(!id&&!password)return showMsg('userProfileMsg','Para criar um usuário, informe uma senha com pelo menos 8 caracteres.');if(password&&password.length<8)return showMsg('userProfileMsg','A senha deve ter pelo menos 8 caracteres.');showMsg('userProfileMsg',id?'Salvando usuário...':'Criando usuário...');const r=await callUserAdmin(id?'update':'create',{user_id:id||undefined,email,full_name:name,role,is_active:active,password:password||undefined,permissions});if(r.error)return showMsg('userProfileMsg',r.error);showMsg('userProfileMsg',id?'Usuário atualizado com sucesso.':'Usuário criado com sucesso.');setTimeout(()=>clearUserProfile(),700);await refreshUsers();}
async function deleteUserProfile(){const id=$('userProfileId').value;if(!id)return showMsg('userProfileMsg','Selecione um usuário antes de excluir.');const email=$('userProfileEmail').value.trim();if(!confirm(`Excluir permanentemente o usuário ${email||'selecionado'}?\n\nIsso remove a conta de acesso. Para apenas bloquear o acesso, desmarque “Usuário ativo” e salve.`))return;showMsg('userProfileMsg','Excluindo usuário...');const r=await callUserAdmin('delete',{user_id:id});if(r.error)return showMsg('userProfileMsg',r.error);showMsg('userProfileMsg','Usuário excluído.');clearUserProfile();await refreshUsers();}
function clearUserProfile(){setUserForm({role:'consulta',is_active:true},{...ROLE_DEFAULTS.consulta});$('userProfileId').value='';$('userProfileEmail').value='';$('userProfilePassword').value='';$('deleteUserProfile').disabled=true;showMsg('userProfileMsg','');}
async function syncUsers(){const msg=$('userProfileMsg');if(msg) msg.textContent='Sincronizando contas do Supabase Auth...';const r=await client.rpc('aelo_sync_auth_profiles');if(r.error){if(msg) msg.textContent=r.error.message;return;}if(msg) msg.textContent=`${r.data||0} conta(s) sincronizada(s).`;await refreshUsers();}
function bindUserRoleDefaults(){const role=$('userProfileRole');if(!role||role.dataset.defaultsBound==='1')return;role.dataset.defaultsBound='1';role.addEventListener('change',()=>{applyRoleDefaults(role.value);showMsg('userProfileMsg','Permissões padrão aplicadas. Clique em Salvar usuário.');});}
function initUsersManager(){bindUserRoleDefaults();$('refreshUsers')?.addEventListener('click',refreshUsers);$('syncUsers')?.addEventListener('click',syncUsers);$('newAuthUser')?.addEventListener('click',()=>{clearUserProfile();$('userProfileName')?.focus();});$('saveUserProfile')?.addEventListener('click',saveUserProfile);$('deleteUserProfile')?.addEventListener('click',deleteUserProfile);$('clearUserProfile')?.addEventListener('click',clearUserProfile);clearUserProfile();refreshUsers();}
...
$("cep")?.addEventListener("blur",async e=>{
  const cep=formatCep(e.target.value); e.target.value=cep;
  ...
  try{
    ...
    if(d.erro) return;
    const loc=$("location");
    if(loc && !loc.value.trim()){ loc.value=[d.bairro,d.localidade,d.uf].filter(Boolean).join(" • "); }
  }catch(err){ console.debug("CEP AELO:",err); }
});
