...
async function saveUserProfile(){const id=$('userProfileId').value;if(!id)return showMsg('userProfileMsg','Selecione um usuário existente do Supabase Auth.');const profile={user_id:id,full_name:$('userProfileName').value.trim(),email:$('userProfileEmail').value.trim(),role:$('userProfileRole').value,is_active:$('userProfileActive').checked,updated_at:new Date().toISOString()};const a=await client.from('user_profiles').upsert(profile,{onConflict:'user_id'});if(a.error)return showMsg('userProfileMsg',a.error.message);const b=await client.from('user_permissions').upsert({...getUserPermissionPayload(),user_id:id,updated_at:new Date().toISOString()},{onConflict:'user_id'});if(b.error)return showMsg('userProfileMsg',b.error.message);await client.from('audit_log').insert({user_id:(await client.auth.getUser()).data.user.id,action:'atualizou_permissoes',module:'usuarios',record_id:id,details:{role:profile.role}});showMsg('userProfileMsg','Permissões salvas.');refreshUsers();}
...
function initUsersManager(){ $('refreshUsers')?.addEventListener('click',refreshUsers);$('saveUserProfile')?.addEventListener('click',saveUserProfile);$('clearUserProfile')?.addEventListener('click',clearUserProfile);$('userProfileRole')?.addEventListener('change',e=>applyRoleDefaults(e.target.value));refreshUsers();}
